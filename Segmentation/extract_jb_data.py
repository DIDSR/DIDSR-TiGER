...
def extract_jb_data(data_path):
    ...
    msks_names.sort()
    imgs_names = imgs_names[:18]
    msks_names = msks_names[:54]    
    
    imgs_msks_names_masks_coorindates = []
    for i in range(len(imgs_names)):
        temp = imgs_names[i][:-4]
        msk_names = [j for j in msks_names if temp in j]
        msk_coordinates = [j[6:-5] for j in msk_names]
